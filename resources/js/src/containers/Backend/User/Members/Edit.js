...
class Add extends Component {
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { members: { title, edit, index, form } } } }
                }
            },
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faUserTie} title={edit} list={index} link="/user/members" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: photo && `url("${photo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {photo && (photo !== member.photo) && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb items={[{ to: '/user/members', content: index }]} main={edit} icon={faUserTie} />
...
const mapDispatchToProps = dispatch => ({
    get: id => dispatch(actions.getMember(id)),
