...
class Edit extends Component {
    ...
    submitHandler = async e => {
        e.preventDefault();
        await this.props.patch(this.props.match.params.productId, e.target);
    }
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save } }, backend: { pages: { products: { title, edit, index, form } } } }
                }
            },
            backend: { products: { loading, error, message, product } },
            auth: { data: { role: { features } } }
        } = this.props;
        let { name, description, details, link, color, popular, logo } = this.state;
        let content = null;
        let errors = null;
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faProductHunt} title={edit} list={index} link="/user/products" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: logo && `url("${logo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {logo && (logo !== product.logo) && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb items={[{ to: '/user/products', content: index }]} main={edit} icon={faProductHunt} />
...
const mapDispatchToProps = dispatch => ({
    get: id => dispatch(actions.getProduct(id)),
