...
import { faUserTie, faCheckCircle, faFileInvoice, faCalendar, faMoneyBill, faPlus, faMinus } from '@fortawesome/free-solid-svg-icons';
...
class Add extends Component {
    ...
    submitHandler = async e => {
        e.preventDefault();
        await this.props.patch(this.props.match.params.memberId, e.target);
    }
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { invoices: { title, edit, index, form } } } }
                }
            },
            backend: { invoices: { loading, error, message, invoice, customers, tasks } },
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faFileInvoice} title={edit} list={index} link="/user/invoices" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                                ...
                                <Row>
                                    ...
                                    <Col xs={12} className="pb-2 text-large text-700 d-flex justify-content-between">
                                        <div>{form.tasks}</div>
                                        <div>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: photo && `url("${photo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {photo && (photo !== invoice.photo) && <div className="text-center text-green">
        ...
        return <>
            <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                <Breadcrumb items={[{ to: '/user/invoices', content: index }]} main={edit} icon={faFileInvoice} />
...
const mapDispatchToProps = dispatch => ({
    get: id => dispatch(actions.getInvoice(id)),
