...
class Add extends Component {
    ...
    async componentDidMount() {
        this.props.reset();
        this.props.get(this.props.match.params.employeeId);
    }
    ...
    submitHandler = async e => {
        e.preventDefault();
        await this.props.patch(this.props.match.params.employeeId, e.target);
    }
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { employees: { title, edit, index, form } } } }
                }
            },
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faUserTie} title={edit} list={index} link="/user/employees" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: photo && `url("${photo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {photo && (photo !== employee.photo) && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb items={[{ to: '/user/employees', content: index }]} main={edit} icon={faUserTie} />
